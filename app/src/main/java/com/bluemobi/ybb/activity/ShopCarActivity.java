...
//    private double totalMoney;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        titleBarManager.init(ShopCarActivity.this, getSupportActionBar());
        titleBarManager.showTitleTextBar(R.string.str_h_shopping, R.drawable.common_back, R.string.str_edit);
        ...
        if (adapter == null) {
            adapter = new CommonAdapter<ShopCartResponse.ShopCartData.ShopCartDTo>(mContext,
                ...
                public void convert(final ViewHolder helper, final ShopCartResponse.ShopCartData.ShopCartDTo bean) {
                    ...
                    tv_reserveTime.setText(bean.reserveTime+ " " + bean.attributeName +" "+ YbbUtils.getCategoryById(
                            bean.categoryId
                    ));
...
//                break;
        ...
        for (ShopCartResponse.ShopCartData.ShopCartDTo item:dataList){
            if(item.selected){
                if("2".equals(item.categoryId)){
                    ...
                    if(StringUtils.isNotEmpty(preItem.getOrderingTimeType()) &&
                            StringUtils.isNotEmpty(preItem.getOrderingTime())){
                        if("1".equals(preItem.getOrderingTimeType())
                                ){//小时
    ...
    Logger.e("wangzhijun" , current.getTime().toString() +"--->"
    + pre.getTime().toString());
                    if(!current.before(pre)){//不满足条件
                        temp = false;
                        if(pre.get(Calendar.YEAR) == current.get(Calendar.YEAR)//同一天
                                && pre.get(Calendar.MONTH) == current.get(Calendar.MONTH)
                                && pre.get(Calendar.DAY_OF_MONTH) == current.get(Calendar.DAY_OF_MONTH)){
                            Formatter fmt = new Formatter();
                            item.preStatus = "请在" + fmt.format("%tl:%tM", pre, pre)+"之前下单";
        ...
        if(!temp){
            Utils.makeToastAndShow(getBaseContext(), "部分餐品不满足下单条件，请处理后下单");
            adapter.notifyDataSetChanged();
        }
        return temp;
    }
    ...
    private void getTotalPrice()
    {
        ...
        for (int i = 0;i<=dataList.size()-1;i++)
        {
            ...
            if(item.selected){
                ...
                BigDecimal priceDecimal = new BigDecimal(
                        StringUtils.isEmpty(tempPrice) ? "0" : tempPrice
                );
                String tempCount = dataList.get(i).productNum;
                BigDecimal countDecimal = new BigDecimal(
                        StringUtils.isEmpty(tempCount) ? "0" : tempCount
                );
        ...
        totalAmount = total;
        tv_total_money.setText("￥" + String.valueOf(totalAmount.floatValue()));
        Logger.e("wangzhijun", String.valueOf(totalAmount.floatValue()));
    }
...
//                editShopCarDtos.add(val);
            ...
            dao.setCategoryId(item.categoryId);
            dao.setAttributeId(item.attributeId);
            lists.add(dao);
        }
