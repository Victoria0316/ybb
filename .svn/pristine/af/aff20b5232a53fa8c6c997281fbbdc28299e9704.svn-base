...
public class UiUtils
{
    ...
    public static void runOnUiThread(Runnable runnable) {
        // 在主线程运行
        if(android.os.Process.myTid()==YbbApplication.getMainTid()){
            runnable.run();
        }else{
            //获取handler
    ...
    public static void postDelayed(Runnable run, int time) {
        YbbApplication.getHandler().postDelayed(run, time); // 调用Runable里面的run方法
    }
    /**
     * 取消任务
     * @param auToRunTask
    ...
    public static void cancel(Runnable auToRunTask) {
        YbbApplication.getHandler().removeCallbacks(auToRunTask);
    }
