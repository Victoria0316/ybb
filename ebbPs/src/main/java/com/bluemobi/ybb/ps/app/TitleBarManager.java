...
public class TitleBarManager implements View.OnClickListener,CustomSpinner.OnSnipperClickListener {
    ...
    public void setActionBarbg(boolean isTransparent) {
        if (isTransparent) {
            mSupportActionBar.setBackgroundDrawable(activity.getResources().getDrawable(R.color.transparent));
        } else {
            mSupportActionBar.setBackgroundDrawable(activity.getResources().getDrawable(R.color.common_blue));
        }
    ...
    /**
     * 显示最普通的Titlebar 左边按钮，中间文字，右边图片
     */
    public void showBtnTitleTextBar(int textID, int leftSrcID, int rightSrcID,boolean isShowBack) {
        tv_title.setText(textID);
        if (isShowBack) {
            iv_back.setImageResource(leftSrcID);
            iv_back.setVisibility(View.VISIBLE);
